<%-- 
    Document   : organizer-forgot
    Created on : 7 Apr, 2022, 11:03:37 PM
    Author     : SAI PRAKASH
--%>
...
<div class="center">
        ...
        <%
          ...
          ResultSet rs=st.executeQuery("select *from organizer where email='"+name+"' and id='"+roll+"' and question='"+quest+"' and answer='"+ans+"'");
          if(rs.next())
          {
             
              out.println("<center><h3>PASSWORD-CHANGE</h3><table><form action='organizer-update.jsp'>");
              out.println("<tr><td>Password</td><td>:</td><td><input type=password name=password></td></tr>");
              out.println("<tr><td>Re-Password</td><td>:</td><td><input type=password name=re-password></td></tr>");
              out.println("<tr><td colspan=3><input type=hidden value='"+roll+"' name=id></td></tr><tr></tr>");
              out.println("<tr><td ></td><td><input type=submit value=UPDATE id=back style=width:100px;height:50px;background-color:red;border-radius:30px;></td></form><td><button onclick=history.back() style=width:100px;height:50px;background-color:red;border-radius:30px;>BACK</button></td></tr>");
              out.println("</table>");
          }
          else
          {
